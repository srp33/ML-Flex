...
trainTestSvm = function(trainData, testData, kernelType)
{
  ...
  cross = 10
  if (length(trainClasses) < 10)
    cross = length(trainClasses)

  best.parameters = tune.svm(trainData, trainClasses, kernel=kernelType, probability=TRUE, cost=c(0.01,1,100,1000), tunecontrol = tune.control(sampling="cross", cross=cross))$best.parameters
...
trainData = read.table(trainDataFilePath, sep="\t", row.names=NULL, stringsAsFactors=FALSE, header=TRUE, quote="\"", check.names=FALSE)
testData = read.table(testDataFilePath, sep="\t", row.names=NULL, stringsAsFactors=FALSE, header=TRUE, quote="\"", check.names=FALSE)
